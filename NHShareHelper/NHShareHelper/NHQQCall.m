...
- (void)tencentDidLogin {
    ...
    if (_tencentOAuth.accessToken && 0 != [_tencentOAuth.accessToken length])
    {
       ...
       BOOL isGetUserinfo = [_tencentOAuth getUserInfo];
        if (!isGetUserinfo) {
            if (self.delegate && [self.delegate respondsToSelector:@selector(QQRequestUserinfo:errorMsg:)]) {
                [self.delegate QQRequestUserinfo:nil errorMsg:@"调用获取用户信息API失败！"];
            }
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(QQLoginResultSuccess:errorMsg:)]) {
        [self.delegate QQLoginResultSuccess:result errorMsg:error];
    }
}
...
-(void)tencentDidNotLogin:(BOOL)cancelled
{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(QQLoginResultSuccess:errorMsg:)]) {
        [self.delegate QQLoginResultSuccess:NO errorMsg:info];
    }
}
...
{
    if (self.delegate && [self.delegate respondsToSelector:@selector(QQLoginResultSuccess:errorMsg:)]) {
        [self.delegate QQLoginResultSuccess:NO errorMsg:@"无网络连接，请检查网络"];
    }
...
- (void)getUserInfoResponse:(APIResponse*)response{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(QQRequestUserinfo:errorMsg:)]) {
        [self.delegate QQRequestUserinfo:_QQUserinfo errorMsg:response.errorMsg];
    }
}
...
-(NSArray *)getAuthorizedPermissions:(NSArray *)permissions withExtraParams:(NSDictionary *)extraParams{
    NSLog(@"%s---permissions:%@",__func__,permissions);
    return _permissions;
}
...
//        egLog: QQ分享后的回调结果：code 0  errorDescription (null)  infoType (null)
        [[NSNotificationCenter defaultCenter] postNotificationName:ShareResultNotification object:msg.result];
        if (self.delegate && [self.delegate respondsToSelector:@selector(QQShareResultSuccess:errorMsg:)]) {
            BOOL result = YES;
            if (![msg.result isEqualToString:@"0"]) {
                result = NO;
            }
            [self.delegate QQShareResultSuccess:result errorMsg:msg.errorDescription];
        }
