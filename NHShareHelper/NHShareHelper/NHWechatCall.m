...
- (BOOL)wechatRegistWithAppID:(NSString *)appID{
   ...
   return [WXApi registerApp:appID withDescription:[NSBundle mainBundle].bundleIdentifier];
}

- (void)wechatLoginRequestSetViewController:(UIViewController *)viewController{
//    _accessToken = kGetLocalUserinfo(kLocal_key_access_token);
//    _openId = kGetLocalUserinfo(kLocal_key_openid);
...
- (void)completeAuth:(SendAuthResp *)resp{
    ...
    //获取token
    [NHApiRequestHelper getWXTokenWithResp_code:resp.code OpenId:_appID Complete:^(id responseObject, NSError *error) {
        /**
         {
        ...
        if (responseObject && !error) {
            ...
            if (self.delegate && [self.delegate respondsToSelector:@selector(wechatLoginResultSuccess:errorMsg:)]) {
                [self.delegate wechatLoginResultSuccess:YES errorMsg:nil];
            }
            
        }else{
            NHTipWithMessage(@"授权失败,未获取到token");
            if (self.delegate && [self.delegate respondsToSelector:@selector(wechatLoginResultSuccess:errorMsg:)]) {
                [self.delegate wechatLoginResultSuccess:NO errorMsg:@"授权失败,未获取到token"];
            }
        }
    }];
}
...
- (void)completeShare:(id)resp{
    if ([resp isKindOfClass:SendMessageToWXResp.class]) {
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(wechatShareResultSuccess:errorMsg:)]) {
            BOOL result = wxresp.errCode;
            [self.delegate wechatShareResultSuccess:!result errorMsg:wxresp.errStr];
        }
    }
}
...
- (void)getWechatUserinfo{
    ...
    [NHApiRequestHelper getWXUserinfoWithAccessToken:_access_token OpenId:_open_id Complete:^(id responseObject, NSError *error) {
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(wechatRequestUserinfo:error:)]) {
            [self.delegate wechatRequestUserinfo:weakself.wechatUserinfo error:error];
        }
    }];
}
